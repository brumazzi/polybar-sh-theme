...
int show_i3_desktop(int argc, char** argv){
	...
	FILE* sockPath_F = popen("i3 --get-socketpath", "r");
	char sockPath[1024];
	fgets(sockPath, 1024, sockPath_F);
	sockPath[strlen(sockPath)-1] = 0;
	char command[2048];
	sprintf(command, "i3-msg -s \"%s\" -t get_workspaces  | jq -r '.[] | select(.focused==true).name'", sockPath);
	FILE* desktop_F = popen(command, "r");
	...
	for(int i=0; i<argc-2; i++){
		...
		items[i]->callback([](Fl_Widget* widget, void* data){
			...
			if(!fork_pid){
				...
				fgets(sockPath+1, 1023, sockPath_F);
				sockPath[0] = '"';
				sockPath[strlen(sockPath)-1] = '"';
				fclose(sockPath_F);

				char param[8];
				sprintf(param, "%d", radio->flag_value);
				// const char* args[] = {"i3-msg", "-s", (const char*) sockPath, "workspace", param, NULL};
...
int show_sound_card(int argc, char** argv){
	...
	for (int i = 0; i < cards_count; i++) {
		...
		items[i]->callback([](Fl_Widget* widget, void* data) {
				...
				fprintf(selected_F, "%s|%s|%d", radio->icon, radio->label(), radio->flag_value);
				fclose(selected_F);
				}
		});
